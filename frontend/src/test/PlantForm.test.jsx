...
describe("PlantForm", () => {
  ...
  it("renders create mode form", () => {
    ...
    expect(
      screen.getByRole("button", { name: /create plant/i })
    ).toBeInTheDocument();
  });
  ...
  it("validates plant_id max length", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/cannot exceed 64 characters/i)
      ).toBeInTheDocument();
    });
    expect(mockOnSubmit).not.toHaveBeenCalled();
  });
  ...
  it("submits valid form data", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/plant id/i), {
      target: { value: "new_plant" },
    });
