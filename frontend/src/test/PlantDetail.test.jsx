...
describe('PlantDetail', () => {
  ...
  it('should render telemetry chart when data available', async () => {
    ...
    await waitFor(() => {
      ...
      const chartContainer = screen.getByText('Test Plant').closest('.plant-detail');
  ...
  it('should auto-refresh every 30 seconds', async () => {
    ...
    expect(apiClient.fetchPlantDetail).toHaveBeenCalledTimes(1);
    expect(apiClient.fetchTelemetry).toHaveBeenCalledTimes(1);
    expect(apiClient.fetchCommands).toHaveBeenCalledTimes(1);

    // Fast-forward 30 seconds
    vi.advanceTimersByTime(30000);

    await waitFor(() => {
      expect(apiClient.fetchPlantDetail).toHaveBeenCalledTimes(2);
      expect(apiClient.fetchTelemetry).toHaveBeenCalledTimes(2);
      expect(apiClient.fetchCommands).toHaveBeenCalledTimes(2);
    });
